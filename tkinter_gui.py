"""
SearchAlgoViz interactive GUI: build a weighted graph by clicking on the canvas,
then run a search algorithm over it and watch the traversal animate live.

Controls:
    Left-click canvas   Add a node
    Right-click twice    Connect the two closest nodes with a weighted edge
    x                    Clear the canvas
    Set Start / Set Goal  Choose the origin and destination node from the dropdowns

Algorithm keybindings (press after setting Start/Goal, focus the window first):
    1  Depth-First Search          6  Oracle Search (exhaustive)
    2  Breadth-First Search        7  Branch and Bound
    3  Hill Climbing               8  Branch and Bound + Extended List
    4  Beam Search                 9  A* Search
...
class Graph:
    ...
    def __Root_Window(self):
        ...
        legend_text = (
            "Left-click: add node\n"
            "Right-click twice: connect nodes\n"
            "x: clear canvas\n\n"
            "Run algorithm (after Set Start/Goal):\n"
            "1 DFS      6 Oracle\n"
            "2 BFS      7 Branch & Bound\n"
            "3 Hill Climbing  8 B&B + Extended List\n"
            "4 Beam Search  9 A*\n"
            "5 British Museum Search"
        )
        legend_label = Label(self.leftframe, text=legend_text, justify=LEFT, font=('calibre', 9))
        legend_label.grid(row=5, column=0, columnspan=2, pady=20, sticky=W)
