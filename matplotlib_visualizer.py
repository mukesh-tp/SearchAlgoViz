"""
SearchAlgoViz: animated visualizations of classic AI graph-search algorithms
(DFS, BFS, Hill Climbing, Beam Search, Branch and Bound, A*, AO*, and more)
built on networkx and matplotlib.
...
class Algorithm:
    ...
    def AOstar(self, g, o, d):
        """
        AO* search: expands the lowest-cost node by combined path cost and
        heuristic, similar to A* but tracking a closed list of fully expanded nodes.
        Parameters:
            g : is the object of class Graph
            o : origin/start/current node
            d : destination node
...
class GraphVisualization:
    ...
    def visualize_traversal(self, g, o, d, traversal_algorithm, bw=1, title=None, show=True):
        """
        Animates a traversal algorithm step-by-step over the graph, highlighting
        the path explored so far in red on each frame.

        Parameters:
            g: the Graph being traversed
            o: origin/start node
            d: destination node
            traversal_algorithm: bound method on Algorithm (e.g. algo.DFS)
            bw: beam width, only used when traversal_algorithm is Beam Search
            title: figure title (defaults to the algorithm's method name)
            show: whether to call plt.show(); set False for headless/testing use
        """
        G = nx.Graph()
        ...
        fig, ax = plt.subplots()
        fig.suptitle(title or traversal_algorithm.__name__)
        ...
        def update(frame):
            ...
            ax.set_title(f"Step {min(frame + 1, len(paths))}/{len(paths)}")
...
ALGORITHMS = {
    ...
    "3": ("BMS", "British Museum Search"),
    "4": ("HC", "Hill Climbing"),
    "5": ("BS", "Beam Search"),
    "6": ("Oracle", "Oracle Search (exhaustive)"),
    "7": ("OracleH", "Oracle Search (heuristic-guided)"),
    "8": ("BB", "Branch and Bound"),
    "9": ("EL", "Branch and Bound + Extended List"),
    "10": ("EH", "Branch and Bound + Estimated Heuristics"),
    "11": ("Astar", "A* Search"),
    "12": ("BestFirstSearch", "Best-First Search"),
    "13": ("AOstar", "AO* Search"),
}
DEFAULT_CHOICE = "12"
...
def build_default_graph():
    """Builds the classic textbook search-example graph, S -> G."""
    g = Graph()
    g.addEdge('S', 'A', 3)
    g.addEdge('S', 'B', 5)
    g.addEdge('A', 'B', 4)
...
def main():
    ...
    choice = input(f"Pick an algorithm number (Enter for {ALGORITHMS[DEFAULT_CHOICE][1]}): ").strip()
    method_name, label = ALGORITHMS.get(choice, ALGORITHMS[DEFAULT_CHOICE])
    traversal_algorithm = getattr(algo, method_name)

    GraphVisualization().visualize_traversal(g, 'S', 'G', traversal_algorithm, title=label)
